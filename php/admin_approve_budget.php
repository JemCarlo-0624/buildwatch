...
if (!$budget_id || !$proposal_id || !$evaluated_amount || !$status) {
...
try {
    ...
    // === 2. Replace all budget_breakdowns ===
    ...
    // === 3. Record admin evaluation in budget_reviews ===
    ...
    // === 4. Check if evaluated amount > proposed ===
    $budgetCheck = $pdo->prepare("
        SELECT pb.proposed_amount, p.client_id 
    ...
    if ($budgetData && $evaluated_amount > $budgetData['proposed_amount']) {
